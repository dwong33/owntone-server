...
// When removing the workaround then also remove the include of arpa/inet.h
static bool
address_is_trusted_workaround(httpd_backend *backend)
{
  ...
  if (inet_pton(AF_INET, saddr, &naddr.sin.sin_addr) != 1 && inet_pton(AF_INET6, saddr, &naddr.sin6.sin6_addr) != 1)
    return false;
